...
const ADDON='https://raw.githubusercontent.com/gnosisd/virtualmin/aelia-control-engine-v1/barehands-clean/aelia-addons-v2.js';
...
function patch(h,ctrl,addon){
  ...
  h=h.replace('cur.badRun = okBack ? 0 : (cur.badRun || 0) + 1;','cur.badRun = (okBack || aelia.grab) ? 0 : (cur.badRun || 0) + 1;');
  ...
  h=h.replace(palm,`const palmOpen = extFingers >= 4 && ratio > 0.8 && !aelia.backMenuPose;\n        AELIA_CTRL.handleBackMenu(i,cur,aelia,now);`);
  must(h,'cur.softOpen = extFingers >= 3 && ratio > 0.7;','soft open');
  h=h.replace('cur.softOpen = extFingers >= 3 && ratio > 0.7;','cur.softOpen = !aelia.backMenuPose && extFingers >= 3 && ratio > 0.7;');
  ...
  h=h.replace(clawRe,'const claw = aelia.clawPose;');

  const pos='cur.el.style.left = cur.x + "px"; cur.el.style.top = cur.y + "px";';
  must(h,pos,'cursor position');
  h=h.replace(pos,pos+'\n        AELIA_CTRL.handleKnock(i,cur,aelia,lms,now);');

  const oldHint='tap the RING = orbs · tap an orb = its tree · TAP a card = open · pinch-drag = move';
  if(h.includes(oldHint))h=h.replace(oldHint,'LOOK = focus · thumb+index = CLICK · thumb+index+middle = MOVE · BACK HAND = files · INDEX KNOCK = dismiss');
  return h;
}

module.exports=async function(req,res){
  try{
    const headers={'User-Agent':'aelia-control-engine'};
  ...
  }catch(e){res.status(500).send('<pre>AELIA control stage failed: '+String(e.message||e)+'</pre>');}
};
